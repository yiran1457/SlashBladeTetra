...
import se.mickelus.tetra.items.modular.impl.holo.gui.craft.*;
...
public class HoloAddonRootGui extends HoloRootBaseGui {

    public static final char backBinding = 'q';
    private final HoloBreadcrumbsGui breadcrumbs;
    private final HoloAddonItemsGui itemsView;
    ...
    public HoloAddonRootGui(int x, int y) {
        super(x, y);
        this.breadcrumbs = new HoloBreadcrumbsGui(0, 0, this.width, this::onBreadcrumbClick);
        this.addChild(this.breadcrumbs);
        this.itemsView = new HoloAddonItemsGui(0, 70, this.width, this.height, this::onItemSelect, this::onSlotSelect, this::onMaterialsSelect);
    ...
    private void onBreadcrumbClick(int depth) {
        switch (depth) {
            case 0:
                this.onItemSelect((IModularItem)null, (ItemStack)null);
                break;
            case 1:
    ...
    private void onSchematicSelect(UpgradeSchematic schematic) {
        ...
        this.openVariant = null;
        this.schematicView.openVariant((OutcomePreview)null);
        this.schematicsView.setVisible(false);
        this.itemsView.setVisible(false);
        this.showingMaterials = false;
        this.materialsView.setVisible(false);
    ...
    private String getSlotName() {
        if (this.item != null) {
            ...
            for(int i = 0; i < minorKeys.length; ++i) {
                if (minorKeys[i].equals(this.slot)) {
                    return this.item.getMinorModuleNames(this.itemStack)[i];
                }
            }
        }
